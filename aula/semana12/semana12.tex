...
\textit{Dataset: Supermarket Sales — barras, KPI cards, treemap.}\\
...
\textit{Dataset: Video Game Sales — linha temporal, scatter, heatmap.}\\
...
\textit{Dataset: World Happiness Report — mapa coroplético, parâmetro de métrica, filter action.}\\
...
\textit{Dataset: IBM HR Analytics — KPI cards com target, scatter, bullet chart.}\\
\textbf{Semana 12:} Feedback, refinamento, publicação e apresentação executiva.
\textit{Dataset: base escolhida por cada aluno para o T6.}
\end{SolvedBox}
...
\begin{SolvedBox}
\textbf{Questão para o Quadro — Montando a Narrativa SCR}

Você é analista de uma rede de farmácias e precisa convencer o
diretor a investir em um programa de fidelidade. Seus dados mostram:
\begin{itemize}
  \item Taxa de retorno de clientes: apenas 18\% (benchmark do setor: 35\%)
  \item Clientes recorrentes gastam 2{,}8$\times$ mais por visita
  \item 60\% dos cancelamentos ocorrem após a 2\textsuperscript{a} compra
\end{itemize}

Monte a narrativa SCR (Situação $\to$ Complicação $\to$ Resolução)
e defina o próximo passo concreto.

\textbf{Resolução:}
\begin{itemize}
  \item \textbf{S:} ``Nosso índice de retorno é 18\%, metade do benchmark.''
  \item \textbf{C:} ``60\% dos clientes abandonam após a 2\textsuperscript{a}
  compra — estamos perdendo R\$ 2,1M/ano em receita recorrente.''
  \item \textbf{R:} ``Um programa de fidelidade com cashback de 5\% pode
  elevar o retorno para 30\%, gerando R\$ 1,4M adicional por ano.
  Investimento: R\$ 180k em tecnologia + R\$ 60k/ano em cashback.''
  \item \textbf{Próximo passo:} ``Aprovar piloto em 5 lojas no Q3~2026,
  com avaliação de resultados em 90 dias.''
\end{itemize}
\end{SolvedBox}
