...
\begin{SolvedBox}[title=Questão de Fixação 1 - Variáveis Categóricas]
\textbf{Questão:} Loja analisou 1.000 vendas: PIX 45\% (R\$ 85 médio), Crédito 32\% (R\$ 210 médio). Gerente diz: "PIX gera mais receita". Correto?

A) Incorreto; Crédito pode gerar mais (320 × 210 = R\$ 67.200 vs. 450 × 85 = R\$ 38.250)  
B) Correto, 45\% maior  
C) Correto, volume sempre importa  
D) Incorreto, dinheiro é melhor  
E) Correto, sem taxa

\textbf{Resposta:} \textbf{A}  
\textbf{Explicação:} Receita = volume × ticket; Crédito tem ticket maior, gerando mais receita apesar de menor volume.
\end{SolvedBox}

\begin{SolvedBox}[title=Questão de Fixação 2 - Frequência e Proporções]
\textbf{Questão:} Tabela mostra Motivo Cancelamento: Preço 40\%, Mudança 20\%, etc. Qual interpretação?

A) Preço é principal, mas segmentar por idade pode revelar padrões diferentes  
B) Dados inválidos  
C) Óbito é principal  
D) Frequência absoluta só importa  
E) Análise inútil

\textbf{Resposta:} \textbf{A}  
\textbf{Explicação:} Percentuais gerais escondem variações; segmentação revela insights mais profundos.
\end{SolvedBox}
